...
mysql_query("CREATE TABLE IF NOT EXISTS board (
  ...
  post_tripcode varchar(15) NOT NULL,
...
mysql_query("CREATE TABLE IF NOT EXISTS passwords (
  `password` tinytext NOT NULL,
  tripcode tinytext NOT NULL,
  cookies tinytext NOT NULL
) ENGINE=MyISAM DEFAULT CHARSET=utf8");

	if (!mysql_error()) {
		$all_errors .= 'Coздание таблицы &#171;passwords&#187; ОК<br />'."\n\n";
	} else {
		$all_errors .= 'таблица passwords: '.mysql_error().'<br />'."\n\n";
		$error = 1;
	}
...
$check_install = mysql_query("SHOW columns from board_config");
	if ($check_install == 0) {
		...
		$page_title = "Установка эксабы 0.9.7";
		...
		$page_title = "Обновление эксабы на версию 0.9.7";
